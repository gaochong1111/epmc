...
import epmc.propertysolver.ltlqmc.automata.AutomatonBreakpointLabel;
import epmc.propertysolver.ltlqmc.automata.AutomatonSubset;
import epmc.propertysolver.ltlqmc.automata.AutomatonSubsetLabel;
import epmc.propertysolver.ltlqmc.automata.AutomatonSubsetState;
...
public class PropertySolverExplicitLTLQMC implements PropertySolver {
    ...
    private StateMapExplicit checkTemporalLTLNonIncremental(Buechi buechi)
    {
        ...
        if (options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_SUBSET)) {
            result = computeNonIncremental(DecisionMethod.SUBSET, buechi);
        }
        if (result == null && options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_BREAKPOINT)) {
            result = computeNonIncremental(DecisionMethod.BREAKPOINT, buechi);
        }
        if (result == null && options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_RABIN)) {
    ...
    private StateMapExplicit computeNonIncremental(DecisionMethod method, Buechi buechi)
    {
        ...
        switch (method) {
        case SUBSET:
            log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_SUBSET);
            automaton = new AutomatonSubset.Builder().setBuechi(buechi).build();
            break;
        case BREAKPOINT:
            log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_BREAKPOINT);
            automaton = new AutomatonBreakpoint.Builder().setBuechi(buechi).build();
            break;
        case RABIN:
            log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_RABIN);
        ...
        switch (method) {
        case SUBSET:
            log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_SUBSET_DONE);
            break;
        case BREAKPOINT:
            log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_BREAKPOINT_DONE);
            break;
        case RABIN:
            log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_RABIN_DONE);
            break;
        default:
            break;
        }
        log.send(MessagesLTLQMC.LTL_QMC_EXPLORING_STATE_SPACE);
        graph.explore();
        log.send(MessagesLTLQMC.LTL_QMC_EXPLORING_STATE_SPACE_DONE, graph.computeNumStates(),
                automaton.getNumStates());
        log.send(MessagesLTLQMC.LTL_QMC_COMPUTING_END_COMPONENTS);
        ...
        for (BitSet leafSCC = endComponents.next(); leafSCC != null; leafSCC = endComponents.next()) {
            ...
            if (options.getBoolean(OptionsLTLQMC.LTL_QMC_STOP_IF_INIT_DECIDED)) {
            ...
            if (decision == ComponentDecision.ACCEPT) {
                if (options.getBoolean(OptionsLTLQMC.LTL_QMC_REMOVE_DECIDED)) {
        ...
        log.send(MessagesLTLQMC.LTL_QMC_NUM_END_COMPONENTS, numComponents);
        log.send(MessagesLTLQMC.LTL_QMC_NUM_ECC_STATES, numECCStates);
    ...
    {
        log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL);
        ...
        log.send(MessagesLTLQMC.LTL_QMC_INITIALISING_AUTOMATON_AND_PRODUCT_MODEL_DONE);
        log.send(MessagesLTLQMC.LTL_QMC_EXPLORING_STATE_SPACE);
        prodWrapper.explore();
        log.send(MessagesLTLQMC.LTL_QMC_EXPLORING_STATE_SPACE_DONE, prodWrapper.computeNumStates(),
                automaton.getNumStates());
        log.send(MessagesLTLQMC.LTL_QMC_COMPUTING_END_COMPONENTS_INCREMENTALLY);
        ComponentsExplicit components = UtilAlgorithms.newComponentsExplicit();
        EndComponents endComponents = components.maximalEndComponents(prodWrapper);
        BitSet acc = UtilBitSet.newBitSetUnbounded();
        log.send(MessagesLTLQMC.LTL_QMC_COMPUTING_END_COMPONENTS_INCREMENTALLY_DONE);
            ...
            int numLabels, BitSet acc, GraphExplicit graph)
    {
        ...
        for (BitSet ecc = endComponents.next(); ecc != null; ecc = endComponents.next()) {
            ...
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_COMPONENT, numComponents);
            if (options.getBoolean(OptionsLTLQMC.LTL_QMC_REMOVE_DECIDED)) {
                BitSet testSet = ecc.clone();
                testSet.and(acc);
                if (testSet.equals(ecc)) {                    
                    log.send(MessagesLTLQMC.LTL_QMC_SKIPPING_COMPONENT, numComponents);
                    numComponents++;
                    continue;
                }
            }
            ...
            if (decision == ComponentDecision.UNDECIDED
                    && options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_SUBSET)) {
                decision = decideComponentSubset(ecc, graph, numLabels);
            }
            if (decision == ComponentDecision.UNDECIDED
                    && options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_BREAKPOINT)) {
                decision = decideComponentBreakpoint(graph, ecc, bscc);
            }
            if (decision == ComponentDecision.UNDECIDED
                    && options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_RABIN)) {
                decision = decideComponentRabin(graph, ecc, bscc);
            }
            if (decision == ComponentDecision.UNDECIDED
                    && options.getBoolean(OptionsLTLQMC.LTL_QMC_USE_BREAKPOINT_SINGLETONS)) {
            ...
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_COMPONENT_DONE, numComponents);
            numComponents++;
        }
        log.send(MessagesLTLQMC.LTL_QMC_NUM_END_COMPONENTS, numComponents);
        log.send(MessagesLTLQMC.LTL_QMC_NUM_ECC_STATES, numECCStates);
        ensure(undecided.isEmpty(), ProblemsLTLQMC.LTL_QMC_COULDNT_DECIDE);
    }

    private ComponentDecision decideComponentSubset(BitSet ecc, GraphExplicit graph, int numLabels)
    {
        log.send(MessagesLTLQMC.LTL_QMC_DECIDING_SUBSET);
        ...
        if (labelsFoundLower.cardinality() == numLabels) {
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_SUBSET_DONE_ACCEPT);
            return ComponentDecision.ACCEPT;
        } else if (labelsFoundOver.cardinality() != numLabels) {
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_SUBSET_DONE_REJECT);
            return ComponentDecision.REJECT;
        } else {
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_SUBSET_DONE_UNDECIDED);
    ...
    {
        log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT_SINGLETONS);
        ...
        while (!toCheck.isEmpty()) {
            ...
            for (int state = buechi.getNumStates() - 1; state >= 0; state--) {
                ...
                for (BitSet leafSCC = endComponents.next(); leafSCC != null; leafSCC = endComponents.next()) {
                    ...
                    if (decision == ComponentDecision.ACCEPT) {
                        if (bscc) {
                            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT_SINGLETONS_DONE_ACCEPT);
                            return ComponentDecision.ACCEPT;
                        } else {
        ...
        log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT_SINGLETONS_DONE_REJECT);
        return ComponentDecision.REJECT;
    }
            ...
            BitSet ecc, boolean bscc)
    {
        ...
        log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT);
        ...
        for (BitSet leafSCC = endComponents.next(); leafSCC != null; leafSCC = endComponents.next()) {
            ...
            if (decision == ComponentDecision.ACCEPT) {
                if (bscc) {
                    log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT_DONE_ACCEPT);
                    return ComponentDecision.ACCEPT;
                } else {
        ...
        if (undecidedSeen) {
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT_DONE_UNDECIDED);
            return ComponentDecision.UNDECIDED;
        } else {
            log.send(MessagesLTLQMC.LTL_QMC_DECIDING_BREAKPOINT_DONE_REJECT);
            return ComponentDecision.REJECT;
        }
    }
    ...
    private ComponentDecision decideComponentRabin(GraphExplicit subsetProd,
        ...
        log.send(MessagesLTLQMC.LTL_QMC_DECIDING_RABIN);
        ...
        for (BitSet leafSCC = endComponents.next(); leafSCC != null; leafSCC = endComponents.next()) {
            ...
            if (decision == ComponentDecision.ACCEPT) {
                if (bscc) {
                    log.send(MessagesLTLQMC.LTL_QMC_DECIDING_RABIN_DONE_ACCEPT);
                    return ComponentDecision.ACCEPT;
                } else {
        ...
        log.send(MessagesLTLQMC.LTL_QMC_DECIDING_RABIN_DONE_REJECT);
        return ComponentDecision.REJECT;
    }
    ...
    public StateMap solve() {
        ...
        String s = ((ModelPRISMQMC)modelChecker.getModel()).getModelPRISM().toString();
        System.out.println("**************************************");
    ...
    private StateMap doSolve(Expression property, StateSet forStates, boolean min) {
        ...
        if (options.getBoolean(OptionsLTLQMC.LTL_QMC_INCREMENTAL)) {
    ...
    public boolean canHandle() {
      ...
      /*  if (!(modelChecker.getEngine() instanceof EngineExplicit)) {
        ...
        if (!SemanticsQMC.isQMC(semantics)) {
        	return false;
        }
        //TODO 补充完整property需要满足的条件
